...
通用生图模型提供的是“生成能力”，但用户面对空白输入框时，往往不知道应该选择什么风格、如何组织提示词，也难以稳定产出适配真实内容平台的图片。

Xinghe Image 将模型能力封装成可复用的内容生产 Skill：先识别用户的发布场景，再选择预设或推荐视觉方向，最后生成符合目标尺寸和版式要求的图片。
...
flowchart LR
    ...
    D --> E
    E --> F["调用 ERNIE-Image"]
    F --> G["交付图片与 Markdown 引用"]
```

### 产品化重点

- 将 Banner、文章配图、信息图和小红书图文拆成独立工作流
- 用预设库降低空白 Prompt 门槛，用风格库保证视觉表达的可选择性
- 自动处理平台尺寸、中文文字渲染和系列图片一致性
- 同时提供在线体验和 Agent Skill 两种入口
